...
FROM hexpm/elixir:1.12.3-erlang-24.1.7-alpine-3.13.6 as build
...
COPY --from=build --chown="${USER}":"${USER}" /app/_build/"${MIX_ENV}"/rel/plug_http_cache_demo ./

ENTRYPOINT ["bin/plug_http_cache_demo"]
