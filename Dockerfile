...
FROM ubuntu:21.04
RUN apt -y update && apt -y install openssl libncurses5-dev && apt-get clean

WORKDIR "/app"
# Creates an unprivileged user to be used exclusively to run the Phoenix app

#COPY --from=build --chown="${USER}":"${USER}" /app/_build/"${MIX_ENV}"/rel/my_app ./
COPY _build/prod/rel/phoenix_http_cache_demo ./

ENTRYPOINT ["bin/phoenix_http_cache_demo"]
