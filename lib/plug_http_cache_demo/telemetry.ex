defmodule PlugHTTPCacheDemo.Telemetry do
  ...
  def metrics() do
    [
      ...
      counter("http_cache_store_disk.object_deleted.count", event_name: [:http_cache_store_disk, :object_deleted], tag_values: &add_node/1, tags: [:node, :reason]),
      last_value("http_cache_store_disk.memory.total_mem", unit: :byte, tag_values: &add_node/1, tags: [:node]),
      last_value("http_cache_store_disk.memory.objects_mem", unit: :byte, tag_values: &add_node/1, tags: [:node]),
      last_value("http_cache_store_disk.memory.lru_mem", unit: :byte, tag_values: &add_node/1, tags: [:node]),
      last_value("http_cache_store_disk.memory.objects_count", tag_values: &add_node/1, tags: [:node]),
      summary("http_cache_store_disk.lru_nuker.stop.duration", unit: {:native, :microsecond}, tag_values: &add_node/1, tags: [:node]),
      summary("http_cache_store_disk.expired_lru_entry_sweeper.stop.duration", unit: {:native, :microsecond}, tag_values: &add_node/1, tags: [:node]),
      summary("http_cache_store_disk.expired_resp_sweeper.stop.duration", unit: {:native, :microsecond}, tag_values: &add_node/1, tags: [:node]),
